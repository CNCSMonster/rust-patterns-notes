...
| 方面 | 说明 |
|------|------|
| 核心 | `Option<T>` 实现 `IntoIterator` |
| Some 行为 | 迭代一次，产出内部值 |
| None 行为 | 不迭代，直接跳过 |
| 用途 | 避免 `if let Some(x) = opt` 的嵌套 |

**一句话总结**:
> Option 当迭代器用，Some 执行一次，None 跳过，避免模式匹配嵌套。
...
|----------|------|----------|
| 借用捕获 | `\|\| { ... }` | 只读访问，不转移所有权 |
| move 捕获 | `move \|\| { ... }` | 转移所有权到闭包（用于多线程、返回闭包等） |
| 可变借用 | `mut` + `\|\| { ... }` | 需要修改捕获的变量 |

**一句话总结**:
> 闭包默认借用捕获，转移所有权用 `move`，修改变量需要 `mut`。
...
| 方面 | 说明 |
|------|------|
| 核心 | `..Default::default()` 填充未指定字段 |
| 前提 | 类型实现 `Default` trait |
| 优势 | 简洁，只关注需要自定义的字段 |
| 场景 | 配置类结构体、测试数据构造 |

**一句话总结**:
> `..Default::default()` 填充未指定字段，只关注需要自定义的部分，配置构造更简洁。
